...
def handle_tm_language_files():
    ...
    for syntax_file in syntax_files:
        try:
            ...
            print("SimpleCov: could not load {}".format(syntax_file))
            continue

        try:
            extensions = plistlib.readPlistFromBytes(resource).get("fileTypes", [])
        except Exception:
            print("SimpleCov: could not parse {}".format(syntax_file))
            continue
...
def safe_open(filename, mode, *args, **kwargs):
    try:
        ...
        sublime.ok_cancel_dialog("SimpleCov could not access file: \n{}".format(e))
        raise e
    except OSError as e:
        sublime.ok_cancel_dialog("SimpleCov encountered an OS error: \n{}".format(e))
        raise e
